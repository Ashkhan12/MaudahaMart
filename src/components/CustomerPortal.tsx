...
import { ShoppingBasket, Star, MapPin, Clock, Copy, Plus, Minus, Tag, Gift, Sparkles, Send, MessageSquare, ArrowRight, Mic, MicOff, X, Heart } from 'lucide-react';
...
export default function CustomerPortal({
  ...
  // Auto-fill delivery address from active user profile if available
  React.useEffect(() => {
    if (activeUser?.location && !deliveryAddress.trim()) {
      setDeliveryAddress(activeUser.location);
    }
  }, [activeUser]);
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="pt-4 border-t border-slate-100 space-y-3.5">
                      <h4 className="font-extrabold text-xs text-slate-800 flex items-center justify-between">
                        ...
                        <span className="text-[10px] text-emerald-600 font-bold bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200">⚡ Easy Checkout</span>
                      </h4>
                      ...
                      <div>
                        <div className="flex justify-between items-center mb-1.5">
                          ...
                          >
                            📍 {language === 'hi' ? 'नक्शे पर चुनें' : 'Map Picker'}
                          </button>
                        </div>

                        {/* Quick Address Chips for 1-Tap Auto-Fill */}
                        <div className="flex gap-1.5 overflow-x-auto pb-1.5 scrollbar-hide">
                          {activeUser?.location && (
                            <button
                              type="button"
                              onClick={() => setDeliveryAddress(activeUser.location || '')}
                              className="px-2 py-1 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 rounded-lg text-[10px] font-bold text-emerald-700 whitespace-nowrap transition cursor-pointer shrink-0"
                            >
                              🏠 {language === 'hi' ? 'प्रोफाइल का पता' : 'Saved Profile'}
                            </button>
                          )}
                          <button
                            type="button"
                            onClick={() => setDeliveryAddress('Station Road, Near Bus Stand, Maudaha')}
                            className="px-2 py-1 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded-lg text-[10px] font-bold text-slate-700 whitespace-nowrap transition cursor-pointer shrink-0"
                          >
                            📍 Station Road
                          </button>
                          <button
                            type="button"
                            onClick={() => setDeliveryAddress('Main Market, Near Chacha Chauraha, Maudaha')}
                            className="px-2 py-1 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded-lg text-[10px] font-bold text-slate-700 whitespace-nowrap transition cursor-pointer shrink-0"
                          >
                            🏢 Main Market
                          </button>
                        ...
                        <textarea
                          ...
                          placeholder="e.g. Station Road, Near Chacha Chauraha, Maudaha"
                          rows={2}
                          className="w-full bg-slate-50 border border-slate-200 p-2.5 rounded-xl text-xs focus:outline-none focus:border-emerald-500 focus:bg-white transition"
                        />
                      ...
                      <div>
                        ...
                        <div className="py-2.5 px-3.5 bg-emerald-600 text-white rounded-xl text-xs font-black flex items-center justify-between shadow-xs">
                          <span className="flex items-center gap-2">💵 {language === 'hi' ? 'कैश ऑन डिलीवरी (COD)' : 'Cash on Delivery (COD)'}</span>
                          <span className="text-[10px] bg-white/20 px-2 py-0.5 rounded-full font-extrabold">Active</span>
                        </div>
                      </div>

                      {/* Display Coins Earnings */}
                      <p className="text-[10px] text-emerald-700 font-extrabold bg-emerald-50 px-3 py-2 rounded-xl border border-emerald-200 flex items-center gap-1.5">
                        <Sparkles className="h-3.5 w-3.5 text-amber-500 shrink-0" />
                        <span>{t.coinsEarnedMsg.replace('{coins}', Math.floor(subtotal / 20).toString())}</span>
                      </p>

                      <button type="button"
                        onClick={handleCheckoutBtn}
                        className="w-full py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-black text-sm flex items-center justify-center gap-2 transition shadow-lg shadow-emerald-600/20 active:scale-[0.98] cursor-pointer"
                      >
                        <span>{t.placeOrder} • ₹{grandTotal}</span>
