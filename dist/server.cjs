...
app.post("/api/auth/send-otp", async (req, res) => {
  try {
    ...
    const maskedKey = keyInjected ? `${fast2smsKey.trim().substring(0, 4)}...${fast2smsKey.trim().slice(-4)} (Length: ${fast2smsKey.trim().length})` : "NOT CONFIGURED / EMPTY";
    ...
    if (keyInjected) {
      ...
      try {
        ...
        const formattedTo = `+91${tenDigitPhone}`;
        ...
        params.append("Body", `Your Maudaha Mart verification OTP is ${generatedOtp}. Valid for 10 mins.`);
    ...
    console.log(`
======================================================
[Maudaha Mart SMS Gateway] OTP sent to +91 ${tenDigitPhone}
Message: "Your Maudaha Mart verification OTP is ${generatedOtp}. Valid for 10 mins."
...
app.post("/api/auth/verify-otp", (req, res) => {
  try {
    ...
    if (isExpired) {
      console.warn(`[OTP Verify Expired] OTP for ${tenDigitPhone} expired at ${new Date(record.expires).toISOString()}. Current time: ${(/* @__PURE__ */ new Date()).toISOString()}`);
